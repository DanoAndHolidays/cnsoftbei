...
def build(prs):
    ...
    reqs = [
        ("画像", "6 维度对话式构建，随学随新", theme.ACCENT),
        ("资源", "6 类资源多智能体协作生成", theme.ACCENT),
        ("路径", "AI 生成 + 12 预定义结构化路径", theme.PRIMARY),
        ("辅导", "4 模式 + 追问链 + 画像注入", theme.ACCENT_RED),
        ("评估", "真实进度同步 + 能力雷达 + 建议", theme.PRIMARY_DEEP),
    ]
    ...
    principles = [
        ...
        ("多智能体", "5 类智能体职责清晰、可组合", theme.ACCENT),
        ("流式交互", "打字机效果 + 思考过程可视化", theme.ACCENT_RED),
        ("数据闭环", "做题反馈回流画像、画像驱动推荐", theme.PRIMARY_DEEP),
    ]
